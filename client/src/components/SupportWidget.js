...
// ============================================================
// Usage: Add to orders/[id]/page.js:
//   import SupportWidget from "@/components/support/SupportWidget";
//   <SupportWidget orderId={order._id} userId={user._id} token={token} />
// ============================================================
...
export default function SupportWidget({ orderId, userId, token }) {
  ...
  const [step, setStep] = useState("home"); // home | category | waiting | chat
  ...
  useEffect(() => {
    ...
    socketRef.current.on("ticket:status", ({ status, message }) => {
      if (status === "active") {
        setStep("chat");
        setMessages((prev) => [
          ...prev,
          { senderType: "system", message, createdAt: new Date() },
        ]);
      }
    });
  ...
  async function createTicket(cat) {
    ...
    try {
      ...
      // Load messages
      const msgRes = await fetch(`${SERVER}/api/support/tickets/${data._id}/messages`, {
